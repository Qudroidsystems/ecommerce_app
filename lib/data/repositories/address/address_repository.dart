...
class AddressRepository extends GetxController {
  ...
  Future<List<AddressModel>> fetchUserAddresses() async {
    try {
      final response = await THttpHelper.get('/addresses');
      if (response['success']) {
  ...
  Future<String> addAddress(AddressModel address) async {
    try {
      final response = await THttpHelper.post('/addresses', address.toJson());
      if (response['success']) {
  ...
  Future<void> updateAddress(AddressModel address) async {
    try {
      final response = await THttpHelper.put('/addresses/${address.id}', address.toJson());
      if (!response['success']) {
  ...
  Future<void> updateSelectedField({required String addressId, required bool selected}) async {
    try {
      final response = await THttpHelper.patch('/addresses/$addressId', {'is_default': selected});
      if (!response['success']) {
  ...
  Future<void> deleteAddress(String addressId) async {
    try {
      final response = await THttpHelper.delete('/addresses/$addressId');
      if (!response['success']) {
